    ...
    write(...args) {
        ...
        for (let i = 0; i < args.length; i++) {
            ...
            if (arg.text !== undefined) {
                ...
                const style = Object.assign(Object.assign({}, arg.style), { index: [row.length, row.length + txt.length] });
    ...
    print() {
        this.buffer.forEach((row, i) => {
            ...
            row.styleIndex.forEach(style => {
                ...
                const bold = style.bold ? chalk.bold : (_in) => _in;
                const dim = style.dim ? chalk.dim : (_in) => _in;
                const underline = style.underline ? chalk.underline : (_in) => _in;
                const overline = style.overline ? chalk.overline : (_in) => _in;
                const inverse = style.inverse ? chalk.inverse : (_in) => _in;
                const hidden = style.hidden ? chalk.hidden : (_in) => _in;
                const strikethrough = style.strikethrough ? chalk.strikethrough : (_in) => _in;
    ...
    mergeStyles(newStyleIndex, currentStyleIndex, startIndex, newSize) {
        ...
        current.forEach(style => {
            if (style.index[0] < offset && style.index[1] < offset) {
                ...
                merged.push(Object.assign(Object.assign({}, style), { index: [style.index[0], offset] }));
                return;
            }
            else if (style.index[0] < offset && style.index[1] > offset + _newSize) {
                merged.push(Object.assign(Object.assign({}, style), { index: [style.index[0], offset] }));
                merged.push(Object.assign(Object.assign({}, style), { index: [offset + _newSize, style.index[1]] }));
                return;
            }
            else if (style.index[0] >= offset && style.index[1] <= offset + _newSize) {
                // Do nothing
        ...
        new_.forEach(newStyle => {
            merged.push(Object.assign(Object.assign({}, newStyle), { index: [newStyle.index[0] + offset, newStyle.index[1] + offset] }));
        });
